...
function add_header_scroll_script() {
    ...
    body.admin-bar.has-top-banner .ast-primary-header-bar {
        top: 72px !important;  /* 32px (admin bar) + 40px (banner) */
    }
    @media screen and (max-width: 782px) {
        body.admin-bar.has-top-banner .ast-primary-header-bar {
            top: 86px !important;  /* 46px (admin bar) + 40px (banner) */
        }
    }
...
function add_top_promo_banner() {
    ...
    /* 最简方案：给 body 添加 padding-top，所有内容向下移动 */
    body.has-top-banner {
        padding-top: 40px !important;
    }
    
    /* sticky 导航栏在横幅下方 */
    body.has-top-banner header.site-header[style*="position: fixed"],
    body.has-top-banner header.site-header[style*="position: sticky"],
    body.has-top-banner .site-header[style*="position: fixed"],
    body.has-top-banner .site-header[style*="position: sticky"],
    body.has-top-banner .main-header-bar-wrap.ast-header-sticked,
    body.has-top-banner .ast-header-break-point .main-header-bar-wrap.ast-header-sticked .main-header-bar,
    ...
    (function() {
        ...
        function initBanner() {
            ...
            console.log('[Banner] 初始化顶部横幅');
            
            // 添加 body class
            document.body.classList.add('has-top-banner');
            
            // 点击横幅（除了关闭按钮）触发隐藏的弹窗按钮
            banner.addEventListener('click', function(e) {
                if (e.target.classList.contains('banner-close')) {
                    return;
                }
                
                console.log('[Banner] 横幅被点击，查找隐藏的弹窗按钮');
                
                // 查找隐藏的弹窗触发按钮（ID: popup-trigger-12817）
                var popupTriggerContainer = document.getElementById('popup-trigger-12817');
                
                if (popupTriggerContainer) {
                    console.log('[Banner] 找到隐藏按钮容器');
                    
                    // 查找容器内的实际链接元素
                    var popupLink = popupTriggerContainer.querySelector('a');
                    
                    if (popupLink) {
                        console.log('[Banner] 找到按钮内的链接，触发点击');
                        popupLink.click();
                    } else {
                        // 如果没有链接，直接点击容器
                        console.log('[Banner] 未找到链接元素，直接点击容器');
                        popupTriggerContainer.click();
                    }
                } else {
                    console.error('[Banner] 未找到隐藏的弹窗按钮（ID: popup-trigger-12817）');
                    console.log('[Banner] 请确保在页面中添加了带有此 ID 的 Elementor 按钮');
                }
            });
